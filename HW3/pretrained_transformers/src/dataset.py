...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Step 0: Get the document at the given index
        document = self.data[idx]

        # Step 1: Randomly truncate the document to length between 4 and int(block_size*7/8)
        max_len = int(self.block_size * 7 / 8)
        min_len = 4

        # Make sure document is long enough
        if len(document) < min_len:
            # If document is too short, pad it or handle edge case
            document = document + self.PAD_CHAR * (min_len - len(document))

        # Truncate to a random length
        truncate_len = random.randint(min_len, min(len(document), max_len))
        document = document[:truncate_len]

        # Step 2: Break document into [prefix] [masked_content] [suffix]
        # masked_content should be 1/4 of the document length on average
        # We'll pick a random length for masked_content
        doc_len = len(document)

        # Pick masked_content length: random around 1/4 of doc_len
        # Use a range that averages to 1/4
        masked_len = random.randint(1, max(1, doc_len // 2))  # This averages to ~1/4

        # Pick a random start position for the masked content
        max_start = doc_len - masked_len
        if max_start < 0:
            max_start = 0
        start_idx = random.randint(0, max_start)

        prefix = document[:start_idx]
        masked_content = document[start_idx:start_idx + masked_len]
        suffix = document[start_idx + masked_len:]
        ...
        # Step 4: Create input (x) and output (y) pairs
        # x = masked_string[:-1], y = masked_string[1:]
        x = masked_string[:-1]
        y = masked_string[1:]
